...
public class MyDaoImpl implements MyDao {
	...
	public String loginAction(loginBean log) {
		...
		if (log.getEmail().equals("ad@admin.com")) {
			...
			String sql = "select fname,lname,email from user where email='"
					+ log.getEmail() + "' and password= '" + log.getPassword()
					+ "' ";
			try {
				user u = template.queryForObject(sql,
	...
	public int reGistration(user u) {
		...
		String sql = "insert into user(fname,lname,email,password,sex,dob,phone) "
				...
				+ u.getSex()
				+ "','"
				+ new java.sql.Date(u.getDob().getTime())
				+ "','"
				+ u.getPhone() + "')";
	...
	public List<user> showEmp() {
		...
		List<user> u = template.query(sql, new BeanPropertyRowMapper<user>(
				user.class));
		return u;
	}
	...
	public user showEmpForUpdate(String email) {
		email = email + "%";
		String sql = "select fname,lname,email,sex,dob,phone FROM user WHERE email like '"
				+ email + "'";
	...
	public userAddress InsertEmpAddressUpdate(String email, String addType) {
		...
		String sql = "select email,address_type,address,city,pin,state,country FROM user_address WHERE email like '"
				+ email + "' and address_type = '" + addType + "'";
	...
	public void updateInformation(user u) {
		String sql = "update user set dob='"
				+ new java.sql.Date(u.getDob().getTime()) + "',sex='"
				+ u.getSex() + "',phone='" + u.getPhone() + "' where email='"
				+ u.getEmail() + "'";
		template.update(sql);
	}
	...
	public void insertAddressInformation(userAddress userAdd) {
		...
		userAddress a = this.InsertEmpAddressUpdate(userAdd.getEmail(),
				userAdd.getAddType());
		if (a == null) {
			...
			String sql = "Update user_address set address = '"
					+ userAdd.getAddress() + "', city = '" + userAdd.getCity()
					+ "', pin = '" + userAdd.getPin() + "', state = '"
					+ userAdd.getState() + "', country ='"
					+ userAdd.getCountry() + "' ";
			sql += "Where email like '" + email + "' and address_type = '"
					+ userAdd.getAddType() + "'";
			template.update(sql);
		}
	}
